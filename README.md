...
AstraSync Wallet is an open-source browser plugin for the DeFi ecosystem, providing users with a better-to-use and more secure multi-chain experience.

## Install

You can download the latest Rabby [here](https://github.com/RabbyHub/Rabby/releases/latest).

## Guideline for Integrating AstraSync Wallet

To help dapp developers support and integrate AstraSync Wallet more easily, we recommend using our integration solution that has almost NO development cost and does not introduce any uncertainty:
...
1. On your connection page, display both connection buttons for "MetaMask" and "AstraSync Wallet" when the "Ethereum" object is detected. These two buttons basically have the same function. Users can click either of them to interact with the "Ethereum" object and perform the connection operation. These two buttons are only used to display both brands' logos to help users understand their operation path.
2. If the "Ethereum" object is not detected, then suggest that users download the extension wallet and provide download links for both "MetaMask" and "AstraSync Wallet."
...
According to the above solution, if a user is using the "AstraSync Wallet" and clicks the "connect to MetaMask" button, they will still interact with the "AstraSync Wallet" and vice versa, which might be a bit weird.
